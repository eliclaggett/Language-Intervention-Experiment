...
import { SendRounded, Warning, FastForwardRounded } from "@mui/icons-material";
import {
  usePlayer,
  useGame,
  useStageTimer,
...
export default function Conversation({ next }) {
  ...
  const game = useGame();
  const { experimentCondition, suggestionAlgorithm } = game.get("treatment");
  ...
  const opinionStrength = opinionSurveyResponses
    ? opinionSurveyResponses[topic]
    : 0;
  ...
  // Empirica chat and typing channels
  ...
  useEffect(() => {
    ...
    if (messages && messages.length >= 2) {
      ...
      wsSend(
        JSON.stringify({
          ...
          treatmentAlgorithm: suggestionAlgorithm,
        }),
        true
      );
    }
    ...
    window.nlpServer.onmessage = (msg) => {
      ...
      let currentSuggestion =
        typeof window.suggestion === "undefined" ||
        window.suggestion == "undefined"
          ? ""
          : window.suggestion;
      ...
      if (partial_reply != "[EOS]") {
        ...
        if (currentSuggestion.slice(0, 4).toLowerCase() == "you:") {
          currentSuggestion = currentSuggestion
            .slice(currentSuggestion.indexOf(":") + 1)
            .trim();
        }
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
          if (messages[i].sender == playerId) {
            if (messages[i].txt === messages[i].txt.toLowerCase())
              convertToLowercase = true;
            break;
          }
        }
        if (convertToLowercase)
          currentSuggestion = currentSuggestion.toLowerCase();
        ...
        const censor = new TextCensor().setStrategy(
          keepStartCensorStrategy(asteriskStrategy)
        );
  ...
  useEffect(() => {
    if (typeof messages == "object" && "length" in messages) {
      ...
      const histMsgs =
        messages.length > 1 ? [messages[0], messages[1]] : [{}, {}];
      histMsgs[0].personId = -1;
      histMsgs[1].personId = -1;
      ...
      for (const msg of messages) {
        ...
        uiElems.push(
          <Message
            ...
            avatarPosition="tl"
          >
            <Message.Header sender={senderTxt} />
            {avatar}
          </Message>
        );
        msgIdx++;
      ...
      if (
        numPlayerMsgs >= gameParams.numMsgsFinishEarly &&
        !player.get("requestEarlyFinish") &&
        !player.get("forceEarlyFinish")
      ) {
        setSkipDisplay("flex");
      }
    }
  ...
  const acknowledgeGroupUI = (
    ...
    >
      <Typography level="body-md">
        We finished grouping like-minded participants and assigning conversation
        partners.
      </Typography>
      <Typography level="h1">Your partner is from {groupTxt} group.</Typography>
      {groupGraphic}
      <Typography level="body-md">
        Next, you will chat with your partner about an assigned topic.
      </Typography>
      <Button onClick={goToChat} sx={{ mb: 4 }}>
        Continue to Chat
      </Button>
    </Stack>
  );

  const readInstructionsUI = (
  ...
  const suggestionClass =
    autocompleteOptions[0].length > 0 && autocompleteOptions[0] != "undefined"
      ? "msgSend treatment"
      : "msgSend treatment hidden";
  ...
  if (
    msgUnderReview.length > 0 &&
    autocompleteOptions[0].length > 0 &&
    autocompleteOptions[0] != "undefined"
  ) {
    msgSendUI = (
    ...
    rewriteUI = (
      <div className="msgSend treatment">
        <span onClick={handleRewriteClick}>
          Suggested rephrasing (click to edit)
        </span>
        <div className="input-wrapper">
  ...
  if (window.requestRewrite) {
    msgSendUI = <div className="msgSend">Analyzing message...</div>;
    rewriteUI = "";
  }
  ...
  if ((forceEarlyFinish || player.get("requestEarlyFinish")) && stageTimer) {
    timeLeft =
      gameParams.cooperationDiscussionTime * 1000 * 60 -
      (earlyFinishTimeLeft - timeLeft);
    if (timeLeft <= 0) {
      next();
    }
  } else if (currentStage == "cooperationDiscussion") {
    timeLeft =
      (gameParams.chatTime + gameParams.cooperationDiscussionTime) * 1000 * 60 -
      elapsed;
    if (timeLeft <= 0) {
      next();
    }
    ...
    >
      ...
      >
        <Box sx={{ flex: 1 }}>
          ...
          <Typography level="body-md">
            If you do not actively participate in this chat, you will be removed
            from this study.
          </Typography>
        </Box>
      </Alert>
      <MainContainer style={{ overflow: "visible" }}>
        <ChatContainer style={{ height: "25rem" }}>
          <MessageList typingIndicator={typingIndicator}>{msgsUI}</MessageList>
        </ChatContainer>
        {msgSendUI}
        {rewriteUI}
        {autocompleteUI}
        <div className="msgSend">
          <Alert
            ...
            color="neutral"
            invertedColors
            startDecorator={<FastForwardRounded />}
            style={{
              alignItems: "flex-start",
              display:
                currentStage != "cooperationDiscussion" ? skipDisplay : "none",
            }}
          >
            <Box sx={{ flex: 1, mt: -0.25 }}>
              <Typography level="body-md">
                Thanks for your active participation! You may end the chat early
                if you'd like.
              </Typography>
              <Box
                sx={{
  ...
  return <Container maxWidth="100vw">{ui}</Container>;
}
