/*
 * Filename: SemiSynchronousSteps.jsx
 * Author: Elijah Claggett
 *
 * Description:
 * This ReactJS file defines the timed steps of the experiment
 */
...
export default function SemiSynchronousSteps() {
  ...
  // State variables to track participants' progress through the experiment
  const chatEnded =
    player.get("serverChatEnded") || player.get("processEarlyFinish") || false;
  const acknowledgedChatEnd = player.get("passedChat") || false;
  const submittedCooperationDecision =
    player.get("submitCooperationDecision") || false;
  const submittedReflectionSurvey =
    player.get("submitReflectionSurvey") || false;
  ...
  // Helper function to proceed through the experiment
  function handleNext() {
    setStep(step + 1);
    return "Loading...";
  }
